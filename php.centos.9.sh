...
# PHP 7.4 (and PHP 8.0) are not compatible with OpenSSL 3. 
# Either use PHP 8.1, or stick with OpenSSL 1.1 with these older PHP versions.
read -p "Enter php version like 8.1.29,8.2.21: " PHP_VERSION
...
function install_libzip {
    ...
    cd $SRC_DIR
    wget $LIBZIP_DOWN -O $LIBZIP_SRC.tar.gz
...
function install_common {
    ...
    yum install -y sudo wget gcc gcc-c++ make sudo autoconf gd-devel \
        freetype-devel libxml2-devel libjpeg-devel libpng-devel openssl-devel \
        sqlite-devel libtool libzip-tools \
